...
const FindOrCreateTicketService = async (
  ...
  if (!ticket) {
    // Guard anti-race: evita criar ticket duplicado se já existe um criado nos últimos 30s
    // para o mesmo contato+conexão (proteção contra Baileys entregar a mesma mensagem 2x)
    const recentTicket = await Ticket.findOne({
      where: {
        contactId: baseContactId,
        companyId,
        whatsappId: whatsapp.id,
        createdAt: { [Op.gte]: sub(new Date(), { seconds: 30 }) }
      },
      order: [["id", "DESC"]]
    });

    if (recentTicket) {
      logger.info(`[FindOrCreateTicket] Ticket recente encontrado (anti-race): ${recentTicket.id} — reutilizando em vez de criar novo`);
      ticket = recentTicket;
      reusedTicket = true;
    } else {
