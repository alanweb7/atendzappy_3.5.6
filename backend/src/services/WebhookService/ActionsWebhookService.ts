...
import { SendTextOfficialService } from "../WhatsAppOfficial/SendTextOfficialService";
import { SendMediaOfficialService } from "../WhatsAppOfficial/SendMediaOfficialService";
  ...
  sendMessage: async (jid: string, payload: any) => {
    try {
      if (payload.text) {
        ...
        const mediaKey = payload.image ? "image"
          : payload.video ? "video"
          : payload.audio ? "audio"
          : payload.document ? "document"
          : null;
        if (mediaKey && Buffer.isBuffer(payload[mediaKey])) {
          const extMap: Record<string, string> = { image: "jpg", video: "mp4", audio: "ogg", document: "pdf" };
          const mimeMap: Record<string, string> = { image: "image/jpeg", video: "video/mp4", audio: "audio/ogg", document: "application/pdf" };
          const ext = extMap[mediaKey] || "bin";
          const tempFilename = `${randomString(16)}.${ext}`;
          const tempDir = path.join(path.resolve(__dirname, "../../.."), "public", `company${companyId}`);
          if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
          const tempPath = path.join(tempDir, tempFilename);
          const mediaBuf = Buffer.isBuffer(payload[mediaKey]) ? payload[mediaKey] : Buffer.from(payload[mediaKey]);
          fs.writeFileSync(tempPath, mediaBuf as unknown as Uint8Array);
      ...
      console.error("[OfficialWbotShim] Erro ao enviar:", err);
    }
    return { key: { remoteJid: jid, id: randomString(20), fromMe: true } };
  }
});
    ...
    const checkAndCloseTicketIfAssigned = async (ticketId: string) => {
      try {
        ...
        if (ticket && ticket.userId) {
          console.log(`Ticket ${ticketId} está com usuário ${ticket.user?.name}, parando fluxo automático (não fecha ticket)`);
          return true; // Apenas para o fluxo — não fecha o ticket
        }
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "openai") {
        ...
        try {
          ...
          if (cfg.iaMode === "system" && !Number.isNaN(promptIdNumber)) {
            ...
            // Resolver apiKey conforme aiMode do nó
            let resolvedApiKey = apiKey;
            if (cfg.aiMode === "system" || !cfg.aiMode) {
              const wl = await getWhitelabelConfig(companyId);
          ...
          const wbot = whatsapp?.channel === "whatsapp_official"
            ? createOfficialWbotShim(whatsapp, ticket, contact, companyId) as any
            : getWbot(whatsapp.id);

          const officialMsg = whatsapp?.channel === "whatsapp_official" && !msg
            ? { key: { remoteJid: `${numberClient}@s.whatsapp.net`, fromMe: false, id: randomString(20) }, message: { conversation: pressKey || "" }, messageTimestamp: Math.floor(Date.now() / 1000) } as any
            : msg;
      ...
      if (nodeSelected.type === "directOpenai") {
        ...
        try {
          ...
          // Para API Oficial, construir msg shim se não existir
          const officialMsgBody = pressKey || "";
          if (whatsapp?.channel === "whatsapp_official" && !msg && officialMsgBody) {
            msg = { key: { remoteJid: `${numberClient}@s.whatsapp.net`, fromMe: false, id: randomString(20) }, message: { conversation: officialMsgBody }, messageTimestamp: Math.floor(Date.now() / 1000) } as any;
          }
          ...
          if (msg) {
            ...
            const bodyMessage = msg.message?.conversation || msg.message?.extendedTextMessage?.text || (whatsapp?.channel === "whatsapp_official" ? pressKey || "" : "");
      ...
      if (nodeSelected.type === "question") {
        ...
        if (whatsapp?.channel === "whatsapp_official") {
          await SendMessage(whatsapp, { number: numberClient, body: questionMessage, companyId }, false, ticketDetails);
        } else {
          await SendWhatsAppMessage({ body: questionMessage, ticket: ticketDetails, quotedMsg: null });
        }
        ...
        if (enableQueuePosition) {
          ...
          if (whatsapp?.channel === "whatsapp_official") {
            await SendMessage(whatsapp, { number: numberClient, body: bodyFila, companyId }, false, ticketFilaDetails);
          } else {
            await SendWhatsAppMessage({ body: bodyFila, ticket: ticketFilaDetails, quotedMsg: null });
          }
      ...
      if (nodeSelected.type === "singleBlock") {
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          if (elementNowSelected.includes("audio")) {
            ...
            if (whatsapp?.channel === "whatsapp_official") {
              await SendMessage(whatsapp, { number: numberClient, body: "", mediaPath: mediaDirectory, companyId }, false, ticketInt);
            } else {
          ...
          if (elementNowSelected.includes("video")) {
            ...
            if (whatsapp?.channel === "whatsapp_official") {
              ...
              await typeSimulation(ticket, "composing");
              await SendWhatsAppMediaFlow({ media: mediaDirectory, ticket: ticketInt });
            }
            await intervalWhats("1");
          }
      ...
      if (nodeSelected.type === "file") {
        ...
        try {
          if (whatsapp?.channel === "whatsapp_official") {
            await SendMessage(whatsapp, { number: numberClient, body: "", mediaPath: mediaDirectory, companyId }, false, ticket);
          } else {
            await typeSimulation(ticket, "composing");
            await SendWhatsAppMediaFlow({ media: mediaDirectory, ticket: ticket });
            await typeSimulation(ticket, "paused");
          }
          console.log("Arquivo enviado com sucesso");
        } catch (error) {
      ...
      if (nodeSelected.type === "removeTag") {
        ...
        if (tagId && ticket) {
          try {
            await TicketTag.destroy({ where: { ticketId: ticket.id, tagId }, individualHooks: true });
            await ContactTag.destroy({ where: { contactId: ticket.contactId, tagId } });

            const io = getIO();
            const ticketUpdated = await ShowTicketService(ticket.id, companyId);
            io.of(String(companyId)).emit(`company-${companyId}-ticket`, { action: "update", ticket: ticketUpdated });
          } catch (error: any) {
            console.error(`removeTag: Erro ao remover tag normal`, error.message);
          }
        }
      }

      // Nó: Remover Tag Kanban
      if (nodeSelected.type === "removeTagKanban") {
        if (!ticket && idTicket) {
          ticket = await Ticket.findOne({ where: { id: idTicket, companyId } });
        }
        ...
        if (ticket) {
          try {
            ...
            if (kanbanTagIds.length > 0) {
              await TicketTag.destroy({ where: { ticketId: ticket.id, tagId: kanbanTagIds }, individualHooks: true });
            }
            ...
            console.error(`removeTagKanban: Erro ao remover tag kanban`, error.message);
          }
        }
      }
        ...
        if (userId && ticket) {
          ...
          try {
            ...
            if (user) {
              ...
              if (whatsapp?.channel === "whatsapp_official") {
                await SendMessage(whatsapp, { number: numberClient, body: notificationMessage, companyId }, false, ticketUpdated);
              } else {
                await SendWhatsAppMessage({ body: notificationMessage, ticket: ticketUpdated, quotedMsg: null });
              }
            } else {
      ...
      if (nodeSelected.type === "asaas") {
        ...
        if (pressKey && pressKey !== "999") {
          ...
          try {
            ...
            const isOfficialAsaas = whatsapp?.channel === "whatsapp_official";
            const wbot = isOfficialAsaas ? null : getWbot(whatsapp.id);
            const contactNumber = `${ticket?.contact?.number || numberClient}${
              ticket?.isGroup ? "@g.us" : "@s.whatsapp.net"
            }`;

            // Helper local para salvar buffer em disco e enviar via API Oficial
            const sendBufferOfficial = async (buf: Buffer, filename: string, mimetype: string, caption: string) => {
              const tmpDir = path.join(path.resolve(__dirname, "../../.."), "public", `company${companyId}`);
              if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });
              const tmpPath = path.join(tmpDir, filename);
              fs.writeFileSync(tmpPath, buf as unknown as Uint8Array);
              const contactAsaas = await Contact.findOne({ where: { number: numberClient, companyId } });
              await SendMediaOfficialService({
                media: { fieldname: "file", originalname: filename, encoding: "7bit", mimetype, destination: tmpDir, filename, path: tmpPath, size: buf.length, stream: null as any, buffer: null as any } as Express.Multer.File,
                body: caption,
                ticketId: ticket.id,
            ...
            if (boletoFileSource) {
              try {
                ...
                const { buffer: pdfBuffer } = await fetchPdfBufferFromUrl(boletoFileSource);
                const fileName = `boleto-${boletoData.paymentId || "asaas"}.pdf`;
                console.log(`Asaas: Enviando PDF - Tamanho: ${pdfBuffer.length} bytes`);
                if (isOfficialAsaas) {
                  await sendBufferOfficial(pdfBuffer, fileName, "application/pdf", "");
                } else {
                  await wbot.sendMessage(contactNumber, { document: pdfBuffer, fileName, mimetype: "application/pdf" });
                }
                console.log(`Asaas: PDF enviado com sucesso`);
              } catch (pdfError: any) {
                console.error(`Asaas: Erro ao enviar PDF do boleto`, pdfError.message);
                // Fallback: enviar link do boleto
                try {
                  const fallbackText = `📄 Boleto - Vencimento: ${boletoData.dueDate || "N/A"} - Valor: R$ ${boletoData.value?.toFixed(2) || "0.00"}\n\n🔗 Link para o boleto: ${boletoFileSource}\n\n💡 Copie e cole o link no navegador para baixar o PDF.`;
                  if (isOfficialAsaas) {
                    await SendMessageFlow(whatsapp, { number: numberClient, body: fallbackText }, ticket?.id, ticket);
                  } else {
                    await wbot.sendMessage(contactNumber, { text: fallbackText });
                  }
                } catch (linkError: any) {
            ...
            const pixMessage = [];
            if (boletoData.pixCopyPaste) pixMessage.push(`💠 PIX: ${boletoData.pixCopyPaste}`);
            if (boletoLink) pixMessage.push(`🔗 Link: ${boletoLink}`);
            if (pixMessage.length > 0) {
              await SendMessageFlow(whatsapp, { number: numberClient, body: pixMessage.join('\n') }, ticket?.id, ticket);
            }
            ...
            if (boletoData.pixQrCodeImage) {
              try {
                ...
                if (isOfficialAsaas) {
                  await sendBufferOfficial(imageBuffer, `qrcode-${randomString(8)}.jpg`, "image/jpeg", "");
                } else {
                  await wbot.sendMessage(contactNumber, { image: imageBuffer });
                }
              } catch (err) {
      ...
      if (nodeSelected.type === "closeTicket") {
        ...
        if (ticket) {
          ...
          if (message) {
            ...
            if (whatsapp?.channel === "whatsapp_official") {
              await SendMessage(whatsapp, { number: numberClient, body: processedMessage, companyId }, false, ticket);
            } else {
              const wbot = getWbot(whatsapp.id);
              await wbot.sendMessage(`${numberClient}@s.whatsapp.net`, { text: processedMessage });
            }
          ...
          logger.info(`closeTicket: Encerrando ticket ${ticket.id} (status atual: ${ticket.status})`);
          ...
          await Ticket.update(
            { flowWebhook: false, flowStopped: null, lastFlowId: null },
            { where: { id: ticket.id } }
          );

          logger.info(`closeTicket: Ticket ${ticket.id} encerrado com sucesso`);
          return; // para o loop do flow imediatamente
        }
      }
