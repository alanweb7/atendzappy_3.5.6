...
const triggerReopenFlow = async (ticket: any, whatsapp: any, contact: any, msg: any) => {
  try {
    if (ticket.queueId || ticket.userId) return;
    const flowId = whatsapp.flowIdNotPhrase || whatsapp.flowIdWelcome;
    if (!flowId) return;
    const flow = await FlowBuilderModel.findOne({ where: { id: flowId } });
    if (!flow) return;
    const nodes: INodes[] = flow.flow["nodes"];
    const connections: IConnections[] = flow.flow["connections"];
    const mountDataContact = { number: contact.number, name: contact.name, email: contact.email || "" };
    console.log(`[ReopenFlow] Disparando fluxo ${flowId} para ticket reaberto ${ticket.id}`);
    await ActionsWebhookService(
      whatsapp.id, flowId, ticket.companyId,
      nodes, connections, nodes[0]?.id || "1",
      null, "", "", null, ticket.id, mountDataContact, msg || null
    );
  } catch (err) {
    console.error("[ReopenFlow] Erro ao disparar fluxo de reabertura:", err);
  }
};
...
import { isSystemMessage } from "../../helpers/systemMessageCache";
...
const downloadMedia = async (
  ...
  const fileLimit = parseInt(await CheckSettings1("downloadLimit", "2048"), 10);
  if (
    wbot &&
    message?.fileLength &&
...
const verifyContact = async (
  ...
  // ✅ Tentar obter foto com timeout nativo do Baileys
  // Só busca se o contato ainda não tem foto (ou se a foto é inválida)
  // "no_photo" = sentinel para não tentar de novo em todo recebimento de mensagem
  if (wbot) {
    const existingContact = await Contact.findOne({
      where: { number, companyId },
      attributes: ["profilePicUrl"]
    });
    // Ler valor raw do banco (bypassa o getter que converte "no_photo" para "")
    const existingPic: string = (existingContact as any)?.getDataValue?.("profilePicUrl") ?? existingContact?.profilePicUrl ?? "";
    if (existingPic && existingPic !== "no_photo") {
      // Já tem foto válida — reutiliza sem chamar Baileys
      profilePicUrl = existingPic;
    } else if (!existingPic) {
      // Não tem foto — tenta buscar
      try {
        const cdnUrl = await wbot.profilePictureUrl(remoteJid, "image", 5000);
        if (cdnUrl) {
...
const verifyQueue = async (
  ...
  // Verifica se está fora do horário antes de auto-atribuir fila única
  const hasWhatsappSchedules = Array.isArray(whatsapp.schedules) && whatsapp.schedules.length > 0;
  let isCurrentlyOutOfHour = false;
  if (hasWhatsappSchedules && !msg.key.fromMe) {
    const schedCheck = await VerifyCurrentSchedule(companyId, 0, whatsapp.id);
    isCurrentlyOutOfHour = !schedCheck || schedCheck.inActivity === false;
  }

  if (queues.length === 1 && !chatbot && !whatsapp.flowIdWelcome && !isCurrentlyOutOfHour) {
...
const flowbuilderIntegration = async (
  ...
  */
  ...
  if (msg.key.fromMe) {
    // Mensagem enviada pelo lado do atendente — verifica se foi manual (não pelo sistema)
    if (!isSystemMessage(msg.key.id) && (ticket.flowWebhook || ticket.useIntegration)) {
      let pauseMinutes = 10;
      try {
        if (ticket.lastFlowId) {
          const aiFlow = await FlowBuilderModel.findOne({ where: { id: ticket.lastFlowId } });
          if (aiFlow?.flow?.nodes) {
            const aiNode = (aiFlow.flow.nodes as any[]).find(
              (n: any) => n.type === "addDirectOpenAI" || n.data?.openAiSettings
            );
            if (aiNode?.data?.openAiSettings?.pauseMinutes) {
      ...
      await ticket.update({ aiPausedUntil: pausedUntil });
      logger.info(`[AI Pause] Envio manual no ticket ${ticket.id}. IA pausada por ${pauseMinutes} min até ${pausedUntil.toISOString()}`);
    }
    return;
  }
  ...
  const campaignMatchesBody = (campaign: any): boolean => {
    ...
    for (const p of phrases) {
      ...
      if (matchType === "exact") {
        ...
        const phraseWords = phraseNorm
          .replace(/[!?.,"']/g, "")
          .split(/\s+/)
          .filter(w => w.length >= 3);

        if (phraseWords.length > 0) {
          const bodyWords = bodyNorm.replace(/[!?.,"']/g, "").split(/\s+/);
          const allMatch = phraseWords.every(pw =>
            bodyWords.some(bw => bw === pw || bw.includes(pw) || pw.includes(bw))
          );
  ...
  // Keyword flow tem prioridade — se houver match, roda antes de tudo
  const preMatchCampaigns = listPhrase.filter(item => campaignMatchesBody(item));
  if (preMatchCampaigns.length > 0 && !ticket.flowWebhook && !ticket.flowStopped) {
    const flowDisparPre = preMatchCampaigns[0];
    const flowPre = await FlowBuilderModel.findOne({ where: { id: flowDisparPre.flowId } });
    if (flowPre) {
      const mountDataContactPre = { number: contact.number, name: contact.name, email: contact.email };
      await ActionsWebhookService(
        whatsapp.id, flowDisparPre.flowId, ticket.companyId,
        flowPre.flow["nodes"], flowPre.flow["connections"], flowPre.flow["nodes"][0].id,
        null, "", "", null, ticket.id, mountDataContactPre, msg || null
      );
      return;
    }
  }
  ...
  logger.info(`[FlowKeyword] body="${bodyNorm}" | campaigns=${listPhrase.length} | matched=${matchingCampaigns.length}`);
  listPhrase.forEach(c => {
    const ph = c.phrase || "";
    const phs = (() => { try { return JSON.parse(c.phrases || "[]"); } catch { return [c.phrases || ""]; } })();
    logger.info(`[FlowKeyword] campaign id=${c.id} phrase="${ph}" phrases=${JSON.stringify(phs)} matchType=${c.matchType} status=${c.status}`);
  });
...
const handleMessage = async (
  ...
  try {
    ...
    // Para mensagens fromMe: se já existe ticket ativo com contactId diferente (mismatch LID vs número),
    // redirecionar para o contato correto evita que FindOrCreateTicketService crie ticket duplicado.
    let fromMeFixedContact: Contact | null = null;
    if (msg.key.fromMe) {
      ...
      if (openTicket && openTicket.contactId !== contact.id) {
        fromMeFixedContact = await Contact.findByPk(openTicket.contactId);
      }
      // Se não existe ticket ativo → deixa FindOrCreateTicketService criar normalmente
    }
    const effectiveContact = fromMeFixedContact || contact;

    // Verificar status ANTES de FindOrCreateTicketService (que muda closed→pending)
    const preReopenTicket = !msg.key.fromMe ? await Ticket.findOne({
      where: { contactId: groupContact ? groupContact.id : contact.id, companyId, whatsappId: whatsapp.id },
      attributes: ["id", "status", "userId", "queueId"],
      order: [["updatedAt", "DESC"]]
    }) : null;
    const wasClosedWithoutOwner = preReopenTicket?.status === "closed"
      && !preReopenTicket?.userId
      && !preReopenTicket?.queueId;
    ...
    // Disparar fluxo de reabertura logo após FindOrCreateTicketService reabrir o ticket
    if (wasClosedWithoutOwner) {
      // Limpa estado do fluxo anterior para iniciar do zero
      await ticket.update({ flowStopped: null, lastFlowId: null, flowWebhook: false, dataWebhook: null });
      await ticket.reload({ include: [{ model: Whatsapp, as: "whatsapp" }] });
      await triggerReopenFlow(ticket, ticket.whatsapp, contact, msg);
    }
    ...
    if (ticket.isBot && isOpenai && !isNil(flow) && !ticket.queueId && !ticket.userId) {
      ...
      if (ticket.aiPausedUntil && new Date(ticket.aiPausedUntil) > new Date()) {
        logger.info(`[AI Pause] IA pausada até ${ticket.aiPausedUntil} — ignorando mensagem do ticket ${ticket.id}`);
        return;
      }
      ...
      messageBuffer[ticket.id].timeout = setTimeout(async () => {
        try {
          ...
          // Re-verifica o estado atual do ticket — pode ter sido aceito por operador durante o buffer
          const freshTicket = await Ticket.findByPk(ticket.id);
          if (!freshTicket || !freshTicket.isBot || freshTicket.userId) {
            console.log(`🛑 IA cancelada (FlowBuilder) — ticket ${ticket.id} foi aceito por operador`);
            return;
          }
      ...
      messageBuffer[ticket.id].timeout = setTimeout(async () => {
        try {
          ...
          if (!freshTicket || freshTicket.userId) {
            console.log(`🛑 IA cancelada (Conexão) — ticket ${ticket.id} foi aceito por operador`);
            return;
          }
