...
const processSpreadsheetItem = async (
  item: IKnowledgeBaseItem,
  options: ProcessKnowledgeOptions
): Promise<ProcessedKnowledgeBaseItem | null> => {
  try {
    const spreadsheetId = item.url;
    if (!spreadsheetId || !options.companyId) return buildFallbackEntry(item);
    ...
    // Importa inline para evitar circular
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const CompanyGoogleSheetsToken = require("../../models/CompanyGoogleSheetsToken").default;
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { google } = require("googleapis");
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { CheckCompanySetting } = require("../../helpers/CheckSettings");

    const token = tokenId
      ? await CompanyGoogleSheetsToken.findOne({ where: { id: tokenId, companyId: options.companyId } })
      : await CompanyGoogleSheetsToken.findOne({ where: { companyId: options.companyId } });

    if (!token) return buildFallbackEntry(item);

    let clientId = process.env.GOOGLE_CLIENT_ID as string;
    let clientSecret = process.env.GOOGLE_CLIENT_SECRET as string;
    const redirectUri = process.env.GOOGLE_SHEETS_REDIRECT_URI || `${process.env.BACKEND_URL}/google-sheets/oauth-callback`;

    try {
      const sid = await CheckCompanySetting(options.companyId, "googleClientId", "");
      const ss = await CheckCompanySetting(options.companyId, "googleClientSecret", "");
      if (sid) clientId = sid;
    ...
    if (!rows.length) return buildFallbackEntry(item);

    const headers = rows[0];
    const dataRows = rows.slice(1);

    const lines = dataRows.map((row, i) => {
      const pairs = headers.map((h, ci) => `${h}: ${row[ci] || ""}`)
        .filter(p => !p.endsWith(": "))
        .join(" | ");
      return `Linha ${i + 1}: ${pairs}`;
    });

    const summary = `Planilha "${item.title || spreadsheetId}" (aba: ${sheetName})\n` +
    ...
    return {
      ...
      summary,
      metadata: { rows: dataRows.length, columns: headers.length, sheetName }
    };
  } catch (err: any) {
    logger.error(`[AI][KnowledgeBase] Erro ao processar planilha (${item.title || item.url}):`, err.message);
    return null;
  }
};
...
export const processKnowledgeBaseItems = async (
  ...
  for (const item of items) {
    ...
    if (type === "pdf") {
      ...
      processed = await processSpreadsheetItem(item, options);
    }
