...
const DeleteCompanyService = async (id: string): Promise<void> => {
  ...
  try {
    ...
    await runSafeDestroy("CompanyConnectedSheets", async () => CompanyConnectedSheet.destroy({ where: { companyId: id } }));
    await runSafeDestroy("CompanyGoogleSheetsToken", async () => CompanyGoogleSheetsToken.destroy({ where: { companyId: id } }));
