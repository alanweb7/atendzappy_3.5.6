...
const CreatePlanService = async (planData: PlanData): Promise<Plan> => {
  ...
  const dataToSave = {
    ...
    aiCreditsLimit: restData.aiCreditsLimit != null ? Number(restData.aiCreditsLimit) : 0,
    aiCreditsTrial: restData.aiCreditsTrial != null ? Number(restData.aiCreditsTrial) : 0
  };
