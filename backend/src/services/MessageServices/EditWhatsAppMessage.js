"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
var AppError_1 = __importDefault(require("../../errors/AppError"));
var GetTicketWbot_1 = __importDefault(require("../../helpers/GetTicketWbot"));
var Message_1 = __importDefault(require("../../models/Message"));
// import OldMessage from "../../models/OldMessage";
var Ticket_1 = __importDefault(require("../../models/Ticket"));
var EditWhatsAppMessage = function (_a) {
    var messageId = _a.messageId, body = _a.body;
    return __awaiter(void 0, void 0, void 0, function () {
        var message, ticket, wbot, msg, err_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, Message_1["default"].findByPk(messageId, {
                        include: [
                            {
                                model: Ticket_1["default"],
                                as: "ticket",
                                include: ["contact"]
                            }
                        ]
                    })];
                case 1:
                    message = _b.sent();
                    if (!message) {
                        throw new AppError_1["default"]("No message found with this ID.");
                    }
                    ticket = message.ticket;
                    return [4 /*yield*/, (0, GetTicketWbot_1["default"])(ticket)];
                case 2:
                    wbot = _b.sent();
                    msg = JSON.parse(message.dataJson);
                    _b.label = 3;
                case 3:
                    _b.trys.push([3, 8, , 9]);
                    return [4 /*yield*/, wbot.sendMessage(message.remoteJid, {
                            text: body,
                            edit: msg.key
                        }, {})];
                case 4:
                    _b.sent();
                    // await OldMessage.upsert(oldMessage);
                    return [4 /*yield*/, message.update({ body: body, isEdited: true })];
                case 5:
                    // await OldMessage.upsert(oldMessage);
                    _b.sent();
                    return [4 /*yield*/, ticket.update({ lastMessage: body })];
                case 6:
                    _b.sent();
                    return [4 /*yield*/, ticket.reload()];
                case 7:
                    _b.sent();
                    return [2 /*return*/, { ticket: message.ticket, message: message }];
                case 8:
                    err_1 = _b.sent();
                    console.log(err_1);
                    throw new AppError_1["default"]("ERR_EDITING_WAPP_MSG");
                case 9: return [2 /*return*/];
            }
        });
    });
};
exports["default"] = EditWhatsAppMessage;
