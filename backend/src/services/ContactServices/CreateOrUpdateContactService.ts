...
const CreateOrUpdateContactService = async ({
  ...
  try {
    ...
    if (contact) {
      ...
      let profileUrl = profilePicUrl === "no_photo" ? "no_photo" : (profilePicUrl || "");

      if (!profileUrl && wbot && ['whatsapp'].includes(channel)) {
        try {
          profileUrl = await wbot.profilePictureUrl(remoteJid, "image", 5000) || "";
        } catch (e) {
    ...
    if (profilePicUrl && profilePicUrl !== "" && profilePicUrl !== "no_photo" && !profilePicUrl.includes("nopicture.png")) {
