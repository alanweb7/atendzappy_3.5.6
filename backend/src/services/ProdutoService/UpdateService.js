"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
var Produto_1 = __importDefault(require("../../models/Produto"));
var ProdutoCategoria_1 = __importDefault(require("../../models/ProdutoCategoria"));
var ProdutoVariacaoItem_1 = __importDefault(require("../../models/ProdutoVariacaoItem"));
var ProdutoVariacaoOpcao_1 = __importDefault(require("../../models/ProdutoVariacaoOpcao"));
var ProdutoVariacaoGrupo_1 = __importDefault(require("../../models/ProdutoVariacaoGrupo"));
var ProdutoMarca_1 = __importDefault(require("../../models/ProdutoMarca"));
var ProdutoCustomFieldValue_1 = __importDefault(require("../../models/ProdutoCustomFieldValue"));
var ProdutoCustomFieldDefinition_1 = __importDefault(require("../../models/ProdutoCustomFieldDefinition"));
var AppError_1 = __importDefault(require("../../errors/AppError"));
var variacoes_1 = require("./helpers/variacoes");
var syncVariacoes_1 = require("./helpers/syncVariacoes");
var ALLOWED_TYPES = ["fisico", "digital", "imovel", "servico", "veiculo"];
var sanitizeStockValue = function (value) {
    var parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed < 0) {
        return 0;
    }
    return parsed;
};
var UpdateService = function (id, companyId, data) { return __awaiter(void 0, void 0, void 0, function () {
    var produto, categoriaId, categoria, nextTipo, controleEstoque, estoqueAtual, estoqueMinimo, preparedVariacoes, customFieldValues;
    var _a, _b, _c;
    return __generator(this, function (_d) {
        switch (_d.label) {
            case 0: return [4 /*yield*/, Produto_1["default"].findOne({
                    where: {
                        id: id,
                        companyId: companyId
                    }
                })];
            case 1:
                produto = _d.sent();
                if (!produto) {
                    throw new AppError_1["default"]("ERR_PRODUTO_NOT_FOUND", 404);
                }
                categoriaId = data.categoriaId;
                if (!data.hasOwnProperty("categoriaId")) return [3 /*break*/, 5];
                if (!categoriaId) return [3 /*break*/, 3];
                return [4 /*yield*/, ProdutoCategoria_1["default"].findOne({
                        where: {
                            id: categoriaId,
                            companyId: companyId
                        }
                    })];
            case 2:
                categoria = _d.sent();
                if (!categoria) {
                    throw new AppError_1["default"]("ERR_PRODUTO_CATEGORIA_NOT_FOUND", 404);
                }
                return [3 /*break*/, 4];
            case 3:
                categoriaId = null;
                _d.label = 4;
            case 4: return [3 /*break*/, 6];
            case 5:
                categoriaId = produto.categoriaId;
                _d.label = 6;
            case 6:
                nextTipo = (_a = data.tipo) !== null && _a !== void 0 ? _a : produto.tipo;
                if (!ALLOWED_TYPES.includes(nextTipo)) {
                    throw new AppError_1["default"]("ERR_PRODUTO_INVALID_TYPE", 400);
                }
                controleEstoque = produto.controleEstoque;
                estoqueAtual = (_b = produto.estoqueAtual) !== null && _b !== void 0 ? _b : 0;
                estoqueMinimo = (_c = produto.estoqueMinimo) !== null && _c !== void 0 ? _c : 0;
                if (nextTipo === "fisico") {
                    if (data.hasOwnProperty("controleEstoque")) {
                        controleEstoque = Boolean(data.controleEstoque);
                    }
                    if (controleEstoque) {
                        if (data.hasOwnProperty("estoqueAtual")) {
                            estoqueAtual = sanitizeStockValue(data.estoqueAtual);
                        }
                        if (data.hasOwnProperty("estoqueMinimo")) {
                            estoqueMinimo = sanitizeStockValue(data.estoqueMinimo);
                        }
                    }
                    else {
                        estoqueAtual = 0;
                        estoqueMinimo = 0;
                    }
                }
                else {
                    controleEstoque = false;
                    estoqueAtual = 0;
                    estoqueMinimo = 0;
                }
                return [4 /*yield*/, (0, variacoes_1.prepareVariacoes)(companyId, data.variacoes)];
            case 7:
                preparedVariacoes = _d.sent();
                return [4 /*yield*/, produto.update(__assign(__assign({}, data), { tipo: nextTipo, categoriaId: categoriaId, controleEstoque: controleEstoque, estoqueAtual: estoqueAtual, estoqueMinimo: estoqueMinimo }))];
            case 8:
                _d.sent();
                return [4 /*yield*/, (0, syncVariacoes_1.syncVariacoes)(produto.id, preparedVariacoes)];
            case 9:
                _d.sent();
                // Sincronizar campos personalizados
                console.log("UpdateService - data.customFields:", data.customFields);
                console.log("UpdateService - hasOwnProperty customFields:", data.hasOwnProperty("customFields"));
                if (!(data.hasOwnProperty("customFields") && Array.isArray(data.customFields))) return [3 /*break*/, 12];
                console.log("UpdateService - Entrou na condição de customFields");
                // Deletar todos os campos personalizados existentes
                return [4 /*yield*/, ProdutoCustomFieldValue_1["default"].destroy({
                        where: { produtoId: produto.id }
                    })];
            case 10:
                // Deletar todos os campos personalizados existentes
                _d.sent();
                if (!(data.customFields.length > 0)) return [3 /*break*/, 12];
                customFieldValues = data.customFields.map(function (cf) { return ({
                    produtoId: produto.id,
                    fieldDefinitionId: cf.fieldDefinitionId,
                    valorTexto: cf.valorTexto
                }); });
                console.log("UpdateService - Criando customFieldValues:", customFieldValues);
                return [4 /*yield*/, ProdutoCustomFieldValue_1["default"].bulkCreate(customFieldValues)];
            case 11:
                _d.sent();
                _d.label = 12;
            case 12: return [4 /*yield*/, produto.reload({
                    include: [
                        {
                            model: ProdutoCategoria_1["default"],
                            attributes: ["id", "nome"]
                        },
                        {
                            model: ProdutoMarca_1["default"],
                            attributes: ["id", "nome", "logo", "active"]
                        },
                        {
                            model: ProdutoCustomFieldValue_1["default"],
                            include: [
                                {
                                    model: ProdutoCustomFieldDefinition_1["default"],
                                    as: "fieldDefinition"
                                }
                            ]
                        },
                        {
                            model: ProdutoVariacaoItem_1["default"],
                            include: [
                                {
                                    model: ProdutoVariacaoOpcao_1["default"],
                                    attributes: ["id", "nome", "ordem", "grupoId"],
                                    include: [
                                        {
                                            model: ProdutoVariacaoGrupo_1["default"],
                                            attributes: ["id", "nome"]
                                        }
                                    ]
                                }
                            ]
                        }
                    ]
                })];
            case 13:
                _d.sent();
                return [2 /*return*/, produto];
        }
    });
}); };
exports["default"] = UpdateService;
