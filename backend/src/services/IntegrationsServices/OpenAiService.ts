...
import { debitAiInteraction } from "../AiCredits/AiCreditsService";
...
const isToolAllowed = (toolName: string, allowedTools: string[] | null): boolean => {
  ...
  if (toolName === "consultar_base_conhecimento") return true;
...
` : "";
    ...
    try {
      ...
      if (provider === "openai" || provider === "grok") {
        ...
        if (toolCalls.length > 0) {
          // Monta o array correto: [...histórico, assistant_com_tool_calls, tool_responses...]
          const assistantMsg = chat.choices[0].message;

          // Separa mensagens de histórico (não-tool da rodada atual) das tool responses
          const toolResponsesCount = toolCalls.length;
          const historyMsgs = messagesOpenAi.slice(0, messagesOpenAi.length - toolResponsesCount);
          const toolResponseMsgs = messagesOpenAi.slice(messagesOpenAi.length - toolResponsesCount);
        ...
        // Fix: se a IA disse que vai transferir no texto mas não chamou execute_command, forçar a chamada
        const hadQueueTransfer = toolCalls.some(tc => {
          try {
            const tcArgs = JSON.parse(tc.function?.arguments || "{}");
            return tc.function?.name === "execute_command" && (tcArgs.queueId || tcArgs.userId);
          } catch { return false; }
        });

        const transferTextPatterns = [
        ...
        const responseHasTransferText = transferTextPatterns.some(p => p.test(response || ""));

        if (!hadQueueTransfer && responseHasTransferText) {
          logger.info(`[AI] Detectado 'vou transferir' sem execute_command no ticket ${ticket.id} — forçando tool call`);
          try {
            const executeCommandTool = (openAiTools as any[]).find(t => t.function?.name === "execute_command");
            if (executeCommandTool) {
              const forceChat = await aiClient.chat.completions.create({
                model: openAiSettings.model || "gpt-4o",
                messages: [
                  ...messagesOpenAi,
                  {
                ...
                tool_choice: { type: "function" as const, function: { name: "execute_command" } },
                max_tokens: 150,
                temperature: 0
              });

              const forcedCalls = forceChat.choices[0].message?.tool_calls || [];
              for (const fc of forcedCalls) {
                if (fc.type === "function" && fc.function.name === "execute_command") {
                  const fcArgs = JSON.parse(fc.function.arguments || "{}");
                  logger.info(`[AI] Forçando execute_command: ${JSON.stringify(fcArgs)}`);
                  await executeOpenAiTool(
                    "execute_command",
                    fcArgs,
                    ticket,
                    contact,
            ...
            logger.error("[AI] Erro ao forçar execute_command após detectar 'vou transferir':", forceErr);
          }
        }
      } else {
      ...
      // Extrai comandos #{...} do texto antes de sanitizar e os executa após enviar a mensagem
      const inlineCommandsRaw: any[] = [];
      const responseBeforeClean = response || "";
      const commandRegex = /#\{([^}]+)\}/g;
      let cmdMatch;
      while ((cmdMatch = commandRegex.exec(responseBeforeClean)) !== null) {
        try {
          inlineCommandsRaw.push(JSON.parse(`{${cmdMatch[1]}}`));
        } catch { /* parse inválido — ignora */ }
      }

      const sanitizedResponse = sanitizeFinalResponse(responseBeforeClean, contact.name);
      ...
      if (isTextMode || !sendAsAudio) {
        ...
        // Executar comandos inline #{...} extraídos do texto da IA (ex: transferência de fila)
        if (inlineCommandsRaw.length > 0) {
          for (const cmdData of inlineCommandsRaw) {
            try {
              if (cmdData.queueId) {
                const queueId = parseInt(cmdData.queueId);
                const queue = allQueues.find((q: any) => q.id === queueId && q.companyId === ticket.companyId);
                if (queue) {
                  ticket.queueId = queueId;
                  if (cmdData.userId) {
                    const userId = parseInt(cmdData.userId);
                    const user = await User.findOne({ where: { id: userId, companyId: ticket.companyId } });
                    if (user) ticket.userId = userId;
                  }
                  await ticket.save();
                  getIO().of(String(ticket.companyId)).emit(`company-${ticket.companyId}-ticket`, { action: "update", ticket });
                  logger.info(`[AI][InlineCommand] Ticket ${ticket.id} transferido para fila ${queueId}${cmdData.userId ? ` / usuário ${cmdData.userId}` : ""}`);
                }
              } else if (cmdData.userId && !cmdData.queueId) {
                const userId = parseInt(cmdData.userId);
                const user = await User.findOne({ where: { id: userId, companyId: ticket.companyId } });
                if (user) {
                  ...
                  await ticket.save();
                  getIO().of(String(ticket.companyId)).emit(`company-${ticket.companyId}-ticket`, { action: "update", ticket });
                  logger.info(`[AI][InlineCommand] Ticket ${ticket.id} transferido para usuário ${userId}`);
                }
              }
              if (cmdData.resp) {
                await executeOpenAiTool("execute_command", cmdData, ticket, contact, availableTags, allQueues, allowedTools, wbot, firstMsg);
              }
            } catch (err: any) {
              logger.error(`[AI][InlineCommand] Erro ao executar comando inline:`, err.message);
            }
          }
        }
  ...
  // Debita créditos se a empresa está usando a chave global do sistema
  const sysOpenai = process.env.OPENAI_API_KEY;
  const sysGemini = process.env.GEMINI_API_KEY;
  const usesSystemKey =
    (sysOpenai && openAiSettings.apiKey === sysOpenai) ||
    (sysGemini && openAiSettings.apiKey === sysGemini);
  if (usesSystemKey) {
    debitAiInteraction(ticket.companyId).catch(() => {});
  }
};