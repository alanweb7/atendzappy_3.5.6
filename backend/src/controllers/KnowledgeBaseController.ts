...
export const updateItem = async (req: Request, res: Response): Promise<Response> => {
  ...
  const kb = await KnowledgeBase.findOne({ where: { id: knowledgeBaseId, companyId } });
  if (!kb) throw new AppError("Base não encontrada", 404);

  const item = await KnowledgeBaseItem.findOne({ where: { id: itemId, knowledgeBaseId } });
