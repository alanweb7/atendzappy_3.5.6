"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
exports.remove = exports.update = exports.store = exports.show = exports.index = void 0;
var AppError_1 = __importDefault(require("../../errors/AppError"));
var Negocio_1 = __importDefault(require("../../models/Negocio"));
var triggerExternalWebhook_1 = __importDefault(require("../../services/ExternalWebhook/triggerExternalWebhook"));
var ensureExternalAuth = function (req) {
    if (!req.externalAuth) {
        throw new AppError_1["default"]("ERR_EXTERNAL_AUTH_REQUIRED", 401);
    }
    return req.externalAuth;
};
var index = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var companyId, _a, pageNumber, searchParam, limit, offset, whereCondition, _b, count, negocios, hasMore;
    var _c;
    return __generator(this, function (_d) {
        switch (_d.label) {
            case 0:
                companyId = ensureExternalAuth(req).companyId;
                _a = req.query, pageNumber = _a.pageNumber, searchParam = _a.searchParam;
                limit = 50;
                offset = pageNumber ? (Number(pageNumber) - 1) * limit : 0;
                whereCondition = { companyId: companyId };
                if (searchParam) {
                    whereCondition.name = (_c = {},
                        _c[require("sequelize").Op.iLike] = "%".concat(searchParam, "%"),
                        _c);
                }
                return [4 /*yield*/, Negocio_1["default"].findAndCountAll({
                        where: whereCondition,
                        order: [["name", "ASC"]],
                        limit: limit,
                        offset: offset
                    })];
            case 1:
                _b = _d.sent(), count = _b.count, negocios = _b.rows;
                hasMore = count > offset + negocios.length;
                return [2 /*return*/, res.json({ negocios: negocios, count: count, hasMore: hasMore })];
        }
    });
}); };
exports.index = index;
var show = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var companyId, id, negocio;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                companyId = ensureExternalAuth(req).companyId;
                id = req.params.id;
                return [4 /*yield*/, Negocio_1["default"].findOne({
                        where: { id: Number(id), companyId: companyId }
                    })];
            case 1:
                negocio = _a.sent();
                if (!negocio) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NOT_FOUND", 404);
                }
                return [2 /*return*/, res.json(negocio)];
        }
    });
}); };
exports.show = show;
var store = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, _a, name, description, kanbanBoards, users, existingNegocio, negocio;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                _a = req.body, name = _a.name, description = _a.description, kanbanBoards = _a.kanbanBoards, users = _a.users;
                if (!name) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NAME_REQUIRED", 400);
                }
                return [4 /*yield*/, Negocio_1["default"].findOne({
                        where: { name: name, companyId: externalAuth.companyId }
                    })];
            case 1:
                existingNegocio = _b.sent();
                if (existingNegocio) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NAME_ALREADY_EXISTS", 400);
                }
                return [4 /*yield*/, Negocio_1["default"].create({
                        name: name,
                        description: description || null,
                        kanbanBoards: kanbanBoards || [],
                        users: users || [],
                        companyId: externalAuth.companyId
                    })];
            case 2:
                negocio = _b.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "negocio.created",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            negocio: negocio
                        }
                    })];
            case 3:
                _b.sent();
                return [2 /*return*/, res.status(201).json(negocio)];
        }
    });
}); };
exports.store = store;
var update = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, id, _a, name, description, kanbanBoards, users, negocio, existingNegocio;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                id = req.params.id;
                _a = req.body, name = _a.name, description = _a.description, kanbanBoards = _a.kanbanBoards, users = _a.users;
                return [4 /*yield*/, Negocio_1["default"].findOne({
                        where: { id: Number(id), companyId: externalAuth.companyId }
                    })];
            case 1:
                negocio = _b.sent();
                if (!negocio) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NOT_FOUND", 404);
                }
                if (!(name && name !== negocio.name)) return [3 /*break*/, 3];
                return [4 /*yield*/, Negocio_1["default"].findOne({
                        where: { name: name, companyId: externalAuth.companyId }
                    })];
            case 2:
                existingNegocio = _b.sent();
                if (existingNegocio) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NAME_ALREADY_EXISTS", 400);
                }
                _b.label = 3;
            case 3: return [4 /*yield*/, negocio.update({
                    name: name !== undefined ? name : negocio.name,
                    description: description !== undefined ? description : negocio.description,
                    kanbanBoards: kanbanBoards !== undefined ? kanbanBoards : negocio.kanbanBoards,
                    users: users !== undefined ? users : negocio.users
                })];
            case 4:
                _b.sent();
                return [4 /*yield*/, negocio.reload()];
            case 5:
                _b.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "negocio.updated",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            negocio: negocio
                        }
                    })];
            case 6:
                _b.sent();
                return [2 /*return*/, res.json(negocio)];
        }
    });
}); };
exports.update = update;
var remove = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, id, negocio;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                id = req.params.id;
                return [4 /*yield*/, Negocio_1["default"].findOne({
                        where: { id: Number(id), companyId: externalAuth.companyId }
                    })];
            case 1:
                negocio = _a.sent();
                if (!negocio) {
                    throw new AppError_1["default"]("ERR_NEGOCIO_NOT_FOUND", 404);
                }
                return [4 /*yield*/, negocio.destroy()];
            case 2:
                _a.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "negocio.deleted",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            negocioId: Number(id)
                        }
                    })];
            case 3:
                _a.sent();
                return [2 /*return*/, res.status(204).send()];
        }
    });
}); };
exports.remove = remove;
