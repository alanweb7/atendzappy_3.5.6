"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
exports.remove = exports.update = exports.store = exports.show = exports.index = void 0;
var AppError_1 = __importDefault(require("../../errors/AppError"));
var ProjectTask_1 = __importDefault(require("../../models/ProjectTask"));
var ProjectTaskUser_1 = __importDefault(require("../../models/ProjectTaskUser"));
var Project_1 = __importDefault(require("../../models/Project"));
var User_1 = __importDefault(require("../../models/User"));
var triggerExternalWebhook_1 = __importDefault(require("../../services/ExternalWebhook/triggerExternalWebhook"));
var ensureExternalAuth = function (req) {
    if (!req.externalAuth) {
        throw new AppError_1["default"]("ERR_EXTERNAL_AUTH_REQUIRED", 401);
    }
    return req.externalAuth;
};
var index = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var companyId, _a, pageNumber, projectId, status, assignedUserId, limit, offset, whereCondition, includeCondition, _b, count, tasks, hasMore;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                companyId = ensureExternalAuth(req).companyId;
                _a = req.query, pageNumber = _a.pageNumber, projectId = _a.projectId, status = _a.status, assignedUserId = _a.assignedUserId;
                limit = 50;
                offset = pageNumber ? (Number(pageNumber) - 1) * limit : 0;
                whereCondition = { companyId: companyId };
                if (projectId) {
                    whereCondition.projectId = Number(projectId);
                }
                if (status) {
                    whereCondition.status = status;
                }
                includeCondition = [
                    {
                        model: Project_1["default"],
                        as: "project",
                        attributes: ["id", "name", "status"]
                    },
                    {
                        model: User_1["default"],
                        as: "assignedUsers",
                        attributes: ["id", "name", "email"],
                        through: { attributes: [] }
                    }
                ];
                // Filtrar por usuário atribuído
                if (assignedUserId) {
                    includeCondition[1].where = { id: Number(assignedUserId) };
                    includeCondition[1].required = true;
                }
                return [4 /*yield*/, ProjectTask_1["default"].findAndCountAll({
                        where: whereCondition,
                        include: includeCondition,
                        order: [["order", "ASC"], ["createdAt", "DESC"]],
                        limit: limit,
                        offset: offset
                    })];
            case 1:
                _b = _c.sent(), count = _b.count, tasks = _b.rows;
                hasMore = count > offset + tasks.length;
                return [2 /*return*/, res.json({ tasks: tasks, count: count, hasMore: hasMore })];
        }
    });
}); };
exports.index = index;
var show = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var companyId, id, task;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                companyId = ensureExternalAuth(req).companyId;
                id = req.params.id;
                return [4 /*yield*/, ProjectTask_1["default"].findOne({
                        where: { id: Number(id), companyId: companyId },
                        include: [
                            {
                                model: Project_1["default"],
                                as: "project",
                                attributes: ["id", "name", "status", "clientId"]
                            },
                            {
                                model: User_1["default"],
                                as: "assignedUsers",
                                attributes: ["id", "name", "email"],
                                through: { attributes: [] }
                            }
                        ]
                    })];
            case 1:
                task = _a.sent();
                if (!task) {
                    throw new AppError_1["default"]("ERR_TASK_NOT_FOUND", 404);
                }
                return [2 /*return*/, res.json(task)];
        }
    });
}); };
exports.show = show;
var store = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, _a, projectId, title, description, status, order, startDate, dueDate, assignedUserIds, project, task, _i, assignedUserIds_1, userId;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                _a = req.body, projectId = _a.projectId, title = _a.title, description = _a.description, status = _a.status, order = _a.order, startDate = _a.startDate, dueDate = _a.dueDate, assignedUserIds = _a.assignedUserIds;
                if (!projectId) {
                    throw new AppError_1["default"]("ERR_TASK_PROJECT_REQUIRED", 400);
                }
                if (!title) {
                    throw new AppError_1["default"]("ERR_TASK_TITLE_REQUIRED", 400);
                }
                return [4 /*yield*/, Project_1["default"].findOne({
                        where: { id: projectId, companyId: externalAuth.companyId }
                    })];
            case 1:
                project = _b.sent();
                if (!project) {
                    throw new AppError_1["default"]("ERR_PROJECT_NOT_FOUND", 404);
                }
                return [4 /*yield*/, ProjectTask_1["default"].create({
                        projectId: projectId,
                        title: title,
                        description: description || null,
                        status: status || "pending",
                        order: order || 0,
                        startDate: startDate || null,
                        dueDate: dueDate || null,
                        companyId: externalAuth.companyId
                    })];
            case 2:
                task = _b.sent();
                if (!(assignedUserIds && Array.isArray(assignedUserIds))) return [3 /*break*/, 6];
                _i = 0, assignedUserIds_1 = assignedUserIds;
                _b.label = 3;
            case 3:
                if (!(_i < assignedUserIds_1.length)) return [3 /*break*/, 6];
                userId = assignedUserIds_1[_i];
                return [4 /*yield*/, ProjectTaskUser_1["default"].create({
                        projectTaskId: task.id,
                        userId: userId
                    })];
            case 4:
                _b.sent();
                _b.label = 5;
            case 5:
                _i++;
                return [3 /*break*/, 3];
            case 6: return [4 /*yield*/, task.reload({
                    include: [
                        { model: Project_1["default"], as: "project", attributes: ["id", "name", "status"] },
                        { model: User_1["default"], as: "assignedUsers", attributes: ["id", "name", "email"], through: { attributes: [] } }
                    ]
                })];
            case 7:
                _b.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "projectTask.created",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            task: task
                        }
                    })];
            case 8:
                _b.sent();
                return [2 /*return*/, res.status(201).json(task)];
        }
    });
}); };
exports.store = store;
var update = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, id, _a, title, description, status, order, startDate, dueDate, completedAt, assignedUserIds, task, newCompletedAt, _i, assignedUserIds_2, userId;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                id = req.params.id;
                _a = req.body, title = _a.title, description = _a.description, status = _a.status, order = _a.order, startDate = _a.startDate, dueDate = _a.dueDate, completedAt = _a.completedAt, assignedUserIds = _a.assignedUserIds;
                return [4 /*yield*/, ProjectTask_1["default"].findOne({
                        where: { id: Number(id), companyId: externalAuth.companyId }
                    })];
            case 1:
                task = _b.sent();
                if (!task) {
                    throw new AppError_1["default"]("ERR_TASK_NOT_FOUND", 404);
                }
                newCompletedAt = completedAt;
                if (status === "completed" && task.status !== "completed" && !completedAt) {
                    newCompletedAt = new Date();
                }
                return [4 /*yield*/, task.update({
                        title: title !== undefined ? title : task.title,
                        description: description !== undefined ? description : task.description,
                        status: status !== undefined ? status : task.status,
                        order: order !== undefined ? order : task.order,
                        startDate: startDate !== undefined ? startDate : task.startDate,
                        dueDate: dueDate !== undefined ? dueDate : task.dueDate,
                        completedAt: newCompletedAt !== undefined ? newCompletedAt : task.completedAt
                    })];
            case 2:
                _b.sent();
                if (!(assignedUserIds !== undefined && Array.isArray(assignedUserIds))) return [3 /*break*/, 7];
                return [4 /*yield*/, ProjectTaskUser_1["default"].destroy({ where: { projectTaskId: task.id } })];
            case 3:
                _b.sent();
                _i = 0, assignedUserIds_2 = assignedUserIds;
                _b.label = 4;
            case 4:
                if (!(_i < assignedUserIds_2.length)) return [3 /*break*/, 7];
                userId = assignedUserIds_2[_i];
                return [4 /*yield*/, ProjectTaskUser_1["default"].create({
                        projectTaskId: task.id,
                        userId: userId
                    })];
            case 5:
                _b.sent();
                _b.label = 6;
            case 6:
                _i++;
                return [3 /*break*/, 4];
            case 7: return [4 /*yield*/, task.reload({
                    include: [
                        { model: Project_1["default"], as: "project", attributes: ["id", "name", "status"] },
                        { model: User_1["default"], as: "assignedUsers", attributes: ["id", "name", "email"], through: { attributes: [] } }
                    ]
                })];
            case 8:
                _b.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "projectTask.updated",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            task: task
                        }
                    })];
            case 9:
                _b.sent();
                return [2 /*return*/, res.json(task)];
        }
    });
}); };
exports.update = update;
var remove = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var externalAuth, id, task;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                externalAuth = ensureExternalAuth(req);
                id = req.params.id;
                return [4 /*yield*/, ProjectTask_1["default"].findOne({
                        where: { id: Number(id), companyId: externalAuth.companyId }
                    })];
            case 1:
                task = _a.sent();
                if (!task) {
                    throw new AppError_1["default"]("ERR_TASK_NOT_FOUND", 404);
                }
                // Remover usuários atribuídos
                return [4 /*yield*/, ProjectTaskUser_1["default"].destroy({ where: { projectTaskId: task.id } })];
            case 2:
                // Remover usuários atribuídos
                _a.sent();
                return [4 /*yield*/, task.destroy()];
            case 3:
                _a.sent();
                return [4 /*yield*/, (0, triggerExternalWebhook_1["default"])({
                        url: externalAuth.webhookUrl,
                        secret: externalAuth.webhookSecret,
                        event: "projectTask.deleted",
                        data: {
                            apiKeyId: externalAuth.apiKeyId,
                            taskId: Number(id)
                        }
                    })];
            case 4:
                _a.sent();
                return [2 /*return*/, res.status(204).send()];
        }
    });
}); };
exports.remove = remove;
