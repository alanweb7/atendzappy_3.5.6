...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!isNaN(tagListIdNum) && tagListIdNum > 0) {
    ...
    async function createContactListFromTagKanban(tagId: number) {
      ...
      // Tags Kanban associam via TicketTag (ticket → tag), não ContactTag
      const ticketTags = await TicketTag.findAll({
        where: { tagId },
        include: [{ model: Ticket, as: "ticket", attributes: ["contactId"] }]
      });
      const contactIds = [...new Set(
        ticketTags.map(tt => (tt as any).ticket?.contactId).filter(Boolean)
      )];
