...
export const tagHistory = async (req: Request, res: Response): Promise<Response> => {
  const { ticketId } = req.params;
  const { companyId } = req.user;
  const TicketTagHistory = require("../models/TicketTagHistory").default;
  const Tag = require("../models/Tag").default;
  const User = require("../models/User").default;

  const history = await TicketTagHistory.findAll({
    where: { ticketId },
    include: [
