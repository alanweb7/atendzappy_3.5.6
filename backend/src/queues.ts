...
async function getCampaign(id) {
  ...
  if (campaign?.contactListId) {
    const isEmailCampaign = campaign.channel === "email";
    const isOfficialCampaign = campaign.channel === "whatsapp_official";
    // Oficial e email não precisam de isWhatsappValid — aceitam qualquer contato da lista
    const contactWhere = (isEmailCampaign || isOfficialCampaign)
      ? {}
...
async function sendCampaignTemplateBlocks(wbot, chatId, campaign, message, ticket?, contact?, variables: any[] = []) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === "text") {
      ...
      try {
        const { SendCarouselWithFallback } = await import("./helpers/SendCarouselMessage");
        if (ticket) {
          await SendCarouselWithFallback({ ticket, title: block.title || "", cards, companyId: campaign.companyId });
        } else {
          // Sem ticket — usa wbot + chatId direto
          await SendCarouselWithFallback({ wbot, jid: chatId, title: block.title || "", cards, companyId: campaign.companyId });
        }
      } catch (err) {
        logger.error(`❌ Erro ao enviar carrossel na campanha:`, err);
      }
...
async function handlePrepareContact(job) {
  try {
    ...
    if (!contactId) {
      logger.error(`campaignQueue -> PrepareContact -> contactId inválido: ${contactId}`);
      return;
    }

    const campaign = await getCampaign(campaignId);

    // Tenta ContactListItem primeiro; fallback para Contact direto (campanhas antigas)
    let contact: any = await getContact(contactId, false); // ContactListItem
    if (!contact) {
      contact = await getContact(contactId, true); // Contact direto
    }

    if (!contact) {
      logger.warn(`campaignQueue -> PrepareContact -> contato não encontrado: id=${contactId}`);
      return;
    }
    ...
    // Verificar se já existe envio para este número nesta campanha (dedup por número de telefone)
    const alreadySentByNumber = await CampaignShipping.findOne({
      where: {
    ...
    if (alreadySentByNumber) {
      logger.info(`campaignQueue -> PrepareContact -> número ${campaignShipping.number} já disparado/agendado para campanha ${campaignId}, pulando duplicata`);
      return;
    }
...
async function handleDispatchOfficialCampaign(campaign: any, campaignShippingId: number) {
  ...
  try {
    if (campaign.templateName) {
      // Campanha baseada em template aprovado
      ...
      // Campanha de texto/mídia livre (sessão ativa necessária)
      const fakeContact = { id: contactData.id, number: phone, companyId: campaign.companyId } as any;
      const message = campaignShipping.message || "";

      if (campaign.mediaPath) {
        const filePath = path.resolve(process.cwd(), "public", `company${campaign.companyId}`, campaign.mediaPath);
        if (fs.existsSync(filePath)) {
          ...
          logger.warn(`[OfficialCampaign] Arquivo não encontrado: ${filePath}, enviando só texto`);
          await SendTextOfficialService({ body: message, ticketId: 0, contact: fakeContact, connection: campaign.whatsapp });
        }
      } else {
        await SendTextOfficialService({ body: message, ticketId: 0, contact: fakeContact, connection: campaign.whatsapp });
      }
...
async function handleDispatchEmailCampaign(campaign: any, campaignShippingId: number): Promise<void> {
  const campaignShipping = await CampaignShipping.findByPk(campaignShippingId, {
    include: [{ model: Contact, as: "contact", attributes: ["id", "name", "email"] }]
  });

  if (!campaignShipping) {
    logger.error(`campaignQueue -> DispatchEmailCampaign -> campaignShipping ${campaignShippingId} not found`);
    return;
  }

  const contactEmail = campaignShipping.contact?.email || (campaignShipping as any).email;
  if (!contactEmail) {
    logger.warn(`campaignQueue -> DispatchEmailCampaign -> sem email para shipping ${campaignShippingId}`);
    await campaignShipping.update({ deliveredAt: new Date() });
    return;
  }

  const emailAccount = await EmailAccount.findByPk(campaign.emailAccountId);
  if (!emailAccount) {
    logger.error(`campaignQueue -> DispatchEmailCampaign -> emailAccount ${campaign.emailAccountId} not found`);
    return;
  }
  ...
  const contactName = campaignShipping.contact?.name || "Cliente";
  const bodyHtml = (campaign.message1 || "")
  ...
  logger.info(`campaignQueue -> DispatchEmailCampaign -> enviado para ${contactEmail} (shipping ${campaignShippingId})`);
  await campaignShipping.update({ deliveredAt: new Date() });
}
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(`[CAROUSEL DEBUG] Campanha ${campaignId} templateData: ${JSON.stringify(campaign.templateData)?.substring(0, 200)}`);

    // Bifurcação: campanha por email
    if (campaign.channel === "email") {
      await handleDispatchEmailCampaign(campaign, campaignShippingId);
      return;
    }
    ...
    // 1. Tenta ContactListItem (campanhas novas ou convertidas de tag)
    const item = await ContactListItem.findByPk(campaignShipping.contactId);
    if (item) {
      contactData = { id: item.id, name: item.name, number: item.number, email: item.email, isGroup: item.isGroup };
    }

    // 2. Fallback: tenta Contact direto (campanhas de tag antigas onde contactId = Contact.id)
    if (!contactData) {
      const contact = await Contact.findByPk(campaignShipping.contactId, {
        attributes: ["id", "name", "number", "email", "isGroup"]
      });
