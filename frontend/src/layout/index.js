...
const LoggedInLayout = ({ children }) => {
  ...
  const { user, handleLogout, loading, isMobileSession, handleLogin } = useContext(AuthContext);

  // Lógica de impersonação
  ...
  const handleExitImpersonation = async () => {
    ...
    try {
      const { originalToken } = impersonationData;
      // Chama endpoint especial que usa o token original para emitir novo jrt cookie
      const { data } = await api.post("/auth/exit-impersonate", { originalToken });
      localStorage.removeItem("impersonation");
      localStorage.setItem("token", data.token);
      api.defaults.headers.Authorization = `Bearer ${data.token}`;
      window.location.replace("/empresas");
    } catch {
      // Fallback: remove impersonação e vai para login
      localStorage.removeItem("impersonation");
  ...
  const menuGroups = useMemo(
    () => [
      ...
      },
      {
        ...
        disabled: !planActive && location.pathname !== "/financeiro",
        children: [
          { title: "Conversas", path: "/relatorios/conversas" },
          { title: "Atendentes", path: "/relatorios/atendentes" },
          { title: "Tags", path: "/relatorios/tags" },
          { title: "Kanban", path: "/relatorios/kanban" },
          { title: "Faturas", path: "/relatorios/faturas" },
        ],
      },
      {
        title: "Inbox",
        ...
        children: [
          ...
          { title: "Caixa de Entrada", path: "/email-inbox" },
        ],
      },
      {
        title: "CRM",
      ...
      },
      {
        ...
        disabled: !planActive && location.pathname !== "/financeiro",
        children: [
          // { title: "Agente de IA", path: "/agentes" }, // oculto temporariamente
      ...
      },
      {
        ...
        children: [
          ...
          { title: "Lista de disparos", path: "/contact-lists" },
      ...
      // Menu Marketing — visível para admin/gerente ou usuários com canAccessMarketing
      ...((() => {
        const ut = (user?.userType || "").toLowerCase();
        const pf = (user?.profile || "").toLowerCase();
        const isAdminOrManagerLocal = ["administrador","gerente","admin","manager"].includes(ut) || pf === "admin";
        return isAdminOrManagerLocal || user?.canAccessMarketing;
      })() ? [{
        title: "Marketing",
        icon: <CampaignIcon />,
        disabled: !planActive && location.pathname !== "/financeiro",
        children: [
          { title: "Google Meu Negócio", path: "/marketing/google-meu-negocio" },
      ...
      }] : []),
      {
        ...
        disabled: !planActive && location.pathname !== "/financeiro",
        children: [
          { title: "Planilhas", path: "/planilhas" },
        ],
      },
      {
        title: "Financeiro",
        ...
        disabled: !planActive && location.pathname !== "/financeiro",
        children: [
          { title: "Contas", path: "/contas" },
          { title: "Faturas", path: "/faturas" },
          { title: "Financeiro", path: "/financeiro" },
          { title: "Fiscal", path: "/fiscal" },
          { title: "Créditos IA", path: "/creditos-ia" },
        ],
      },
      {
        title: "Ajuda",
      ...
      ...(user?.companyId === 1 ? [{
        ...
        children: [
          ...
          { title: "Créditos IA (Admin)", path: "/admin/creditos-ia", superAdmin: true },
        ],
      }] : []),
    ],
  ...
  const hasMenuAccess = useCallback(
    (path) => {
      if (!path) return false;
      // Páginas acessíveis para todos os usuários autenticados
      if (path === "/painel" || path === "/dashboard" || path === "/tarefas") return true;
  ...
  const filteredMenuGroups = useMemo(() => {
    ...
    return menuGroups
      .map((group) => {
        if (!group.children) {
          // Dashboard é visível para todos os usuários
          if (group.path === "/painel") return group;
    ...
    <div>
      ...
      <div className={classes.sidebarContent}>
        <List className={classes.menuList}>
          ...
          <div className={classes.menuSectionLabel}>Version 3.5.6</div>
        </List>
      </div>
    </div>
  );
  ...
  return (
    <div className={classes.root} style={impersonationData ? { paddingTop: 36 } : {}}>
