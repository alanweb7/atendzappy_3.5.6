...
export function TagsKanbanContainer({ ticket, onStageChange }) {
    ...
    const onChange = async (e) => {
        ...
        // Remove apenas tags kanban — preserva tags normais do ticket
        const kanbanTags = (ticket.tags || []).filter(t => t.kanban === 1);
        for (const kt of kanbanTags) {
            await api.delete(`/ticket-tags/${ticket.id}/${kt.id}`);
        }
        if (value !== null && value !== "") {
