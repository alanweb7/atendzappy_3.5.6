...
const ProtectedRoute = ({ 
  ...
  if (isPublic) {
    ...
    if (!authLoading && isAuth && (rest.path === "/" || rest.path === "/login" || rest.path === "/cadastro")) {
      return <Redirect to={{ pathname: "/painel", state: { from: rest.location } }} />;
    }
  ...
  // Páginas liberadas para todos os usuários autenticados
  const PUBLIC_PATHS = ["/painel", "/dashboard", "/tarefas"];

  // Verifica permissões de página se não for admin
  if (isAuth && user?.profile !== "admin") {
    const pagePath = rest.path;

    // Páginas públicas não verificam permissão
    if (PUBLIC_PATHS.includes(pagePath)) {
      return <RouterRoute {...rest} component={Component} />;
    }

    // Se a página requer permissão específica e o usuário não tem acesso
    if (requiredPermission && requiredPermission !== "none" && !canAccessPage(requiredPermission)) {
      return <Redirect to={{ pathname: "/painel", state: { from: rest.location } }} />;
    }
