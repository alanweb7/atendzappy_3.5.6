...
const FlowBuilderAddDirectOpenAIModal = ({ open, onSave, data, close }) => {
  ...
  const [systemProviders, setSystemProviders] = useState({ openai: false, gemini: false });
    ...
    if (open === "edit") {
      ...
      try {
        ...
        setConfig({
          ...
          pauseMinutes: current.pauseMinutes || 10,
  ...
  useEffect(() => {
    ...
    api.get("/ai-settings").then(({ data }) => setAiIntegrationSettings(Array.isArray(data) ? data : [])).catch(() => {});
    api.get("/ai-credits/system-providers").then(({ data }) => setSystemProviders(data)).catch(() => {});
  }, []);
        ...
        <DialogContent>
          ...
          <TabPanel value={tabValue} index={0} className={classes.tabPanel}>
            <Box className={classes.tabContent}>
              ...
              <Typography variant="h6" style={{ display: 'flex', alignItems: 'center', marginTop: 24, marginBottom: 8 }}>
                <ScheduleIcon style={{ marginRight: 8, color: '#6366f1' }} />
                Pausa da IA ao Responder Manualmente
              </Typography>
              <Typography variant="body2" color="textSecondary" style={{ marginBottom: 16 }}>
                Quando você responder manualmente pelo sistema ou celular, a IA ficará pausada por este tempo antes de voltar a responder automaticamente.
              </Typography>
              <Field
                as={TextField}
                ...
                fullWidth
                className={classes.formControl}
                helperText="Padrão: 10 minutos. Máximo: 1440 (24h)"
              />
            </Box>
          </TabPanel>
          ...
          <TabPanel value={tabValue} index={1} className={classes.tabPanel}>
            <Box className={classes.tabContent}>
              ...
              {/* Modo de uso da IA */}
              <Typography variant="subtitle2" style={{ marginBottom: 6 }}>Modo de uso da IA</Typography>
              <Typography variant="body2" color="textSecondary" style={{ marginBottom: 10 }}>
                Use os créditos incluídos no seu plano ou uma chave de API própria configurada em Integrações.
              </Typography>
              <Box display="flex" style={{ gap: 12, marginBottom: 24 }}>
                <Button
                  variant={values.aiMode === "system" ? "contained" : "outlined"}
                  color={values.aiMode === "system" ? "primary" : "default"}
                  size="small"
                  onClick={() => setFieldValue("aiMode", "system")}
                >
                  Usar créditos do plano
                </Button>
                <Button
                  variant={values.aiMode === "own" ? "contained" : "outlined"}
                  color={values.aiMode === "own" ? "primary" : "default"}
                  size="small"
                  onClick={() => setFieldValue("aiMode", "own")}
                >
                  Usar minha chave de API
                </Button>
              </Box>
              ...
              <FormControl className={classes.formControl} fullWidth>
                ...
                <Select
                  ...
                  label="Provider"
                >
                  {values.aiMode === "system"
                    ? [
                        systemProviders.openai ? <MenuItem key="openai" value="openai">OpenAI</MenuItem> : null,
                        systemProviders.gemini ? <MenuItem key="gemini" value="gemini">Gemini</MenuItem> : null,
                      ].filter(Boolean)
                    : [
                        <MenuItem key="openai" value="openai">OpenAI</MenuItem>,
                        <MenuItem key="gemini" value="gemini">Gemini</MenuItem>,
                        <MenuItem key="grok" value="grok">Grok (xAI)</MenuItem>,
                      ]
                  }
                </Select>
              </FormControl>

              {/* Seletor de API Key — só quando usa chave própria */}
              {values.aiMode === "own" && (() => {
                const keysForProvider = aiIntegrationSettings.filter(s => s.provider === values.provider && s.active);
                return (
                  <FormControl className={classes.formControl} fullWidth>
                    <InputLabel>API Key (Integração)</InputLabel>
                    <Field
                      as={Select}
                      name="apiKey"
                      label="API Key (Integração)"
                    >
                      {keysForProvider.length === 0 ? (
                        <MenuItem value="" disabled>
                          Nenhuma integração {values.provider} configurada
                        </MenuItem>
                      ) : (
                        keysForProvider.map(s => (
                          <MenuItem key={s.id} value={s.apiKey}>
                            {s.provider.charAt(0).toUpperCase() + s.provider.slice(1)} — configurado
                          </MenuItem>
                        ))
                      )}
                    </Field>
                    {keysForProvider.length === 0 && (
                      <Typography variant="caption" style={{ color: "#f44336", marginTop: 4 }}>
                        Configure a integração {values.provider} em Integrações → IA
                      </Typography>
                    )}
                  </FormControl>
                );
