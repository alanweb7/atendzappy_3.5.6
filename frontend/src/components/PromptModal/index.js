...
const PromptModal = ({ open, onClose, promptId }) => {
    ...
    useEffect(() => {
        ...
        api.get("/knowledge-bases")
            ...
            .catch(() => {});
        api.get("/ai-settings")
            .then(({ data }) => setAiIntegrationSettings(Array.isArray(data) ? data : []))
            .catch(() => {});
    }, [open]);
                            ...
                            <Form style={{ width: "100%" }}>
                                ...
                                {activeTab === 2 && (
                                    <>
                                        ...
                                        {/* API Key via integração salva */}
                                        {(() => {
                                            const keysForProvider = aiIntegrationSettings.filter(s => s.provider === selectedProvider && s.active);
                                            return (
                                                <FormControl variant="outlined" margin="dense" fullWidth className={classes.formControl} required>
                                                    ...
                                                    >
                                                        {keysForProvider.length === 0 ? (
                                                            <MenuItem value="" disabled>
                                                                Nenhuma integração {selectedProvider} configurada
                                                            </MenuItem>
                                                        ) : (
                                                    ...
                                                    {keysForProvider.length === 0 && (
                                                        <Typography variant="caption" style={{ color: "#f44336", marginTop: 4 }}>
                                                            Configure a integração em Integrações → IA
                                                        </Typography>
                                                    )}
                                                </FormControl>
                                            );
