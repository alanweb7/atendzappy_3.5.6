...
const QueueIntegration = () => {
  ...
  const [defaultIntegrationType, setDefaultIntegrationType] = useState(null);
  ...
  const [fbIntegrationModalOpen, setFbIntegrationModalOpen] = useState(false);
  const [fbIntegrationId, setFbIntegrationId] = useState(null);
  ...
  const [aiKeyModal, setAiKeyModal] = useState(null); // "openai" | "gemini" | "grok" | null
  const [aiIntegrationSettings, setAiIntegrationSettings] = useState([]);
  ...
  const [sheetsStatus, setSheetsStatus] = useState({ connected: false, email: null, sheets: [] });
  const [sheetsUrlInput, setSheetsUrlInput] = useState("");
  const [sheetsAddingUrl, setSheetsAddingUrl] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchAiSettings = async () => {
      try {
        ...
        setAiIntegrationSettings(Array.isArray(data) ? data : []);
      } catch { }
    };
    ...
    api.get("/google-sheets/status").then(({ data }) => setSheetsStatus(data)).catch(() => {});
  }, []);
  ...
  const handleCloseIntegrationModal = () => {
    setSelectedIntegration(null);
    setDefaultIntegrationType(null);
  ...
  const handleSheetsConnect = async () => {
    try {
      ...
      // Polling para detectar quando o usuário autorizou
      const poll = setInterval(async () => {
        try {
          const res = await api.get("/google-sheets/status");
  ...
  const handleSheetsDisconnect = async () => {
    try {
      await api.delete("/google-sheets/disconnect");
      setSheetsStatus({ connected: false, email: null, sheets: [] });
  ...
  const handleSheetsAddSheet = async () => {
    if (!sheetsUrlInput.trim()) return;
    setSheetsAddingUrl(true);
    try {
      const { data } = await api.post("/google-sheets/sheets", { spreadsheetUrl: sheetsUrlInput });
      setSheetsStatus(prev => ({ ...prev, sheets: [...prev.sheets, data] }));
      setSheetsUrlInput("");
      toast.success("Planilha adicionada!");
    } catch (err) {
      toastError(err);
    } finally {
      setSheetsAddingUrl(false);
    }
  };

  const handleSheetsRemoveSheet = async (sheetId) => {
    try {
      await api.delete(`/google-sheets/sheets/${sheetId}`);
      setSheetsStatus(prev => ({ ...prev, sheets: prev.sheets.filter(s => s.id !== sheetId) }));
  ...
  const getIntegrationInfo = (type) => {
    const integrations = {
      ...
      openai: {
        ...
        bgColor: "#e6f7f3",
        title: "OpenAI",
        description: "Integre modelos GPT-4 para IA conversacional, geração de texto e automação inteligente",
        features: ["GPT-4 e GPT-3.5", "Geração de texto", "Análise de conteúdo"],
      },
      gemini: {
        color: "#4285f4",
        bgColor: "#e8f0fe",
        title: "Google Gemini",
        description: "Use os modelos de IA generativa do Google para potencializar seu atendimento",
        features: ["Gemini Pro e Ultra", "Multimodal (texto e imagem)", "Respostas inteligentes"],
      },
      grok: {
        color: "#1d9bf0",
        bgColor: "#e8f5fd",
        title: "Grok (xAI)",
        description: "IA da xAI com acesso a informações em tempo real e respostas diretas",
        features: ["Respostas em tempo real", "Análise de dados", "Integração com X"],
      },
    };
  ...
  return (
    <Box className={classes.root} onScroll={handleScroll} style={{ "--sidebar-color": sidebarColor }}>
      ...
      <IntegrationModal
        ...
        defaultType={defaultIntegrationType}
      />
      ...
      <FlowBuilderIntegrationModal
        open={fbIntegrationModalOpen}
        integrationId={fbIntegrationId}
        onClose={() => { setFbIntegrationModalOpen(false); setFbIntegrationId(null); }}
        onSave={() => { setFbIntegrationModalOpen(false); setFbIntegrationId(null); }}
      />

      <AIKeyModal
        open={!!aiKeyModal}
        provider={aiKeyModal || "openai"}
        onClose={() => setAiKeyModal(null)}
        onSave={async () => {
          setAiKeyModal(null);
          try {
            const { data } = await api.get("/ai-settings");
        ...
        {loading && queueIntegration.length === 0 ? (
          ...
          <>
            <Box className={classes.cardsGrid}>
                    ...
                    <CardActions style={{ padding: "0 24px 24px", display: "flex", flexDirection: "column", gap: 8 }}>
                      {!hasFlowBuilder ? (
                        <Button
                          ...
                          style={{ backgroundColor: info.color, color: "#fff" }}
                          startIcon={<SettingsIcon />}
                          onClick={() => { setFbIntegrationId(null); setFbIntegrationModalOpen(true); }}
                        >
                          Ativar Integração
                        </Button>
                      ) : (
                        <>
                          <Button
                            ...
                            startIcon={<SettingsIcon />}
                            onClick={() => {
                              const fb = queueIntegration.find(i => i.type === "flowbuilder");
                              setFbIntegrationId(fb?.id || null);
                              setFbIntegrationModalOpen(true);
                            }}
                          >
                            Configurar
                          </Button>
                          <Button
                            ...
                            onClick={async () => {
                              const allFb = queueIntegration.filter(i => i.type === "flowbuilder");
                              if (!allFb.length) return;
                              try {
                                await Promise.all(allFb.map(fb => api.delete(`/queueIntegration/${fb.id}`)));
                                toast.success("Integração Flow Builder desativada!");
                              } catch (err) {
                                toastError(err);
                              }
                            }}
              ...
              {/* Card Google Sheets */}
              {(() => {
                const hasSheets = sheetsStatus.connected;
                return (
                  <Card className={classes.integrationCard}>
                    <CardContent style={{ padding: 24, position: "relative" }}>
                      <Box className={`${classes.statusBadge} ${hasSheets ? classes.activeBadge : classes.inactiveBadge}`}>
                        {hasSheets ? "Ativo" : "Inativo"}
                      </Box>
                      <Box className={classes.iconWrapper} style={{ backgroundColor: "#E8F5E9" }}>
                        <Box style={{ width: 48, height: 48, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>
                          📊
                        </Box>
                      </Box>
                      <Typography className={classes.cardTitle}>Google Sheets</Typography>
                      <Typography className={classes.cardDescription}>
                        Conecte várias planilhas do Google à sua empresa e utilize nos fluxos de atendimento
                      </Typography>
                      {sheetsStatus.accounts?.length > 0 && (
                        <Box style={{ marginTop: 8 }}>
                          {sheetsStatus.accounts.map(acc => (
                            <Typography key={acc.id} style={{ fontSize: 12, color: "#4caf50" }}>
                              ✓ {acc.email}
                            </Typography>
                          ))}
                        </Box>
                      ...
                      <ul className={classes.featureList}>
                        <li><CheckCircleIcon style={{ fontSize: 16, color: "#388e3c" }} /><span>Múltiplas contas Google</span></li>
                        <li><CheckCircleIcon style={{ fontSize: 16, color: "#388e3c" }} /><span>Importar arquivo CSV/XLSX</span></li>
                        <li><CheckCircleIcon style={{ fontSize: 16, color: "#388e3c" }} /><span>Integração com fluxos</span></li>
                      </ul>
                      {sheetsStatus.sheets?.length > 0 && (
                        <Typography style={{ fontSize: 12, color: "#666", marginTop: 4 }}>
                          {sheetsStatus.sheets.length} planilha(s) conectada(s)
                        </Typography>
                      )}
                    </CardContent>
                    <CardActions style={{ padding: "0 24px 24px", display: "flex", flexDirection: "column", gap: 8 }}>
                      <Button variant="contained" className={classes.configButton} style={{ backgroundColor: "#388e3c", color: "#fff" }} startIcon={<SettingsIcon />} onClick={handleSheetsConnect}>
                        Gerenciar Planilhas
                      </Button>
                    </CardActions>
                  </Card>
              ...
              {/* Cards IA: OpenAI, Gemini, Grok */}
              {["openai", "gemini", "grok"].map((provider) => {
                const info = getIntegrationInfo(provider);
                const existing = aiIntegrationSettings.find(item => item.provider === provider);
                const isActive = existing?.active;
                return (
                  <Card key={provider} className={classes.integrationCard}>
                    <CardContent style={{ padding: 24, position: "relative" }}>
                      <Box className={`${classes.statusBadge} ${isActive ? classes.activeBadge : classes.inactiveBadge}`}>
                        {isActive ? "Ativo" : "Inativo"}
                      </Box>
                      <Box className={classes.iconWrapper} style={{ backgroundColor: info.bgColor, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Typography style={{ fontWeight: 800, fontSize: 20, color: info.color, letterSpacing: -1 }}>
                          {info.title.split(" ")[0].slice(0, 2).toUpperCase()}
                        </Typography>
                      </Box>
                      <Typography className={classes.cardTitle}>{info.title}</Typography>
                      <Typography className={classes.cardDescription}>{info.description}</Typography>
                      <ul className={classes.featureList}>
                        {info.features.map((feature, idx) => (
                          <li key={idx}>
                            <CheckCircleIcon style={{ fontSize: 16, color: info.color }} />
                    ...
                    <CardActions style={{ padding: "0 24px 24px", display: "flex", flexDirection: "column", gap: 8 }}>
                      <Button
                        ...
                        style={{ backgroundColor: info.color, color: "#fff" }}
                        startIcon={<SettingsIcon />}
                        onClick={() => setAiKeyModal(provider)}
                      >
                        {existing ? "Configurar" : "Ativar Integração"}
                      </Button>
                    </CardActions>
                  </Card>
                );
