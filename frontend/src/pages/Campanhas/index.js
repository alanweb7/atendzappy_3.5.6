...
const Campaigns = () => {
  ...
  const handleStartNow = async (campaign) => {
    try {
      ...
      toast.success(`Campanha "${campaign.name}" iniciada agora!`);
    } catch (err) {
      toastError(err);
    }
  };
    ...
    <Box className={classes.root} onScroll={handleScroll} style={{ "--sidebar-color": sidebarColor }}>
      ...
      {campaignModalOpen && (
        <CampaignModal
          ...
          initialChannelType={selectedCampaign ? null : selectedChannelType}
        />
      )}
      <EmailCampaignModal
        open={emailCampaignModalOpen}
        onClose={() => { setEmailCampaignModalOpen(false); setSelectedCampaign(null); }}
        campaignId={selectedCampaign?.channel === "email" ? selectedCampaign.id : null}
        onSaved={() => { setPageNumber(1); fetchCampaigns(); }}
      />
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <Box display="flex" justifyContent="center" style={{ gap: 4 }}>
                        ...
                        {/* Iniciar Agora — só para campanhas não iniciadas/canceladas */}
                        {["INATIVA", "CANCELADA"].includes(campaign.status) && (
                          <Tooltip title="Iniciar Agora">
                            <IconButton
                              size="small"
                              onClick={(e) => { e.stopPropagation(); handleStartNow(campaign); }}
                              style={{ color: "#16a34a", backgroundColor: "#f0fdf4", borderRadius: 6 }}
                            >
                              <PlayArrowIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                        )}
                        {/* Cancelar — só para campanhas ativas/programadas */}
                        {["PROGRAMADA", "EM_ANDAMENTO"].includes(campaign.status) && (
                          <Tooltip title="Cancelar Campanha">
                            <IconButton
                              size="small"
                              onClick={(e) => { e.stopPropagation(); handleCancelCampaign(campaign); }}
                              style={{ color: "#dc2626", backgroundColor: "#fef2f2", borderRadius: 6 }}
                            >
