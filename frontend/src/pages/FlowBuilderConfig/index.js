...
import removeTagKanbanNode from "./nodes/removeTagKanbanNode";
...
import FlowBuilderRemoveTagKanbanModal from "../../components/FlowBuilderRemoveTagKanbanModal";
...
const NODE_TITLES = {
  ...
  removeTagKanban: "Remover Tag Kanban",
  condition: "Condição",
...
const nodeTypes = {
  ...
  removeTag: withNodeTitle(removeTagNode, NODE_TITLES.removeTag),
  removeTagKanban: withNodeTitle(removeTagKanbanNode, NODE_TITLES.removeTagKanban),
  ...
  closeTicket: withNodeTitle(closeTicketNode, NODE_TITLES.closeTicket),
...
export const FlowBuilderConfig = () => {
  ...
  const [modalRemoveTag, setModalRemoveTag] = useState(null);
  const [modalRemoveTagKanban, setModalRemoveTagKanban] = useState(null);
      ...
      return setNodes((old) => {
        return [
          ...old,
          {
            id: geraStringAleatoria(30),
            position: { x: posX, y: posY },
            data: withTitleData("removeTag", { ...data }),
            type: "removeTag",
          },
        ];
      });
    }
    if (type === "removeTagKanban") {
      return setNodes((old) => {
        return [
          ...old,
          {
            id: geraStringAleatoria(30),
            position: { x: posX, y: posY },
            data: withTitleData("removeTagKanban", { ...data }),
            type: "removeTagKanban",
          },
        ];
      });
    }
  ...
  const removeTagKanbanAdd = (data) => { addNode("removeTagKanban", data); };
  ...
  const doubleClick = (event, node) => {
    ...
    if (node.type === "removeTag") { setModalRemoveTag("edit"); }
    if (node.type === "removeTagKanban") { setModalRemoveTagKanban("edit"); }
  ...
  const actionGroups = [
    ...
    },
    {
      ...
      actions: [
        ...
        { icon: <LocalOffer sx={{ color: "#ef4444", fontSize: 14 }} />, name: "Remover Tag", type: "removeTag" },
        { icon: <ViewKanban sx={{ color: "#ef4444", fontSize: 14 }} />, name: "Remover Tag Kanban", type: "removeTagKanban" },
  ...
  const clickActions = (type) => {
    switch (type) {
      ...
      case "removeTag": setModalRemoveTag("create"); break;
      case "removeTagKanban": setModalRemoveTagKanban("create"); break;
  ...
  return (
    <Stack sx={{ height: "100vh", backgroundColor: "#f9fafb" }}>
      ...
      <FlowBuilderRemoveTagModal
        ...
        data={dataNode}
        onUpdate={updateNode}
        close={() => setModalRemoveTag(null)}
      />
      <FlowBuilderRemoveTagKanbanModal
        open={modalRemoveTagKanban}
        onSave={removeTagKanbanAdd}
        data={dataNode}
        onUpdate={updateNode}
        close={() => setModalRemoveTagKanban(null)}
      />
