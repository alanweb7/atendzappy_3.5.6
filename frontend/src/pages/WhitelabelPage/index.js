    ...
    async function loadSettings() {
      try {
        ...
        if (Array.isArray(data) && data.length) {
          ...
          setPrivacyText(get("privacyText"));
  ...
  const renderIntegracoes = () => (
    <Grid spacing={3} container>
      <Grid xs={12} item>
        ...
        <Typography variant="body2" className={classes.sectionDesc}>
          Configure as chaves e URLs de integração com serviços externos (Google, etc).
    ...
    <div className={classes.root}>
      ...
      <div className={classes.tabsContainer}>
        ...
        >
          ...
          <Tab label="Política de Privacidade" className={classes.tab} />
        </Tabs>
      </div>
          ...
          <Grid container spacing={2}>
            <Grid xs={12} item>
              ...
              <Typography variant="body2" style={{ color: "#666", marginBottom: 16 }}>
                Este texto será exibido na página de Política de Privacidade do sistema ({`/politica-de-privacidade`}).
              </Typography>
            </Grid>
            <Grid xs={12} item>
              <TextField
                ...
                onBlur={() => { if (privacyText !== settingsLoaded.privacyText) handleSaveSetting("privacyText", privacyText); }}
                placeholder="Digite aqui a política de privacidade do sistema..."
                InputProps={{ style: { backgroundColor: "#ffffff", fontFamily: "monospace", fontSize: 13 } }}
              />
              <Typography variant="caption" style={{ color: "#999", marginTop: 4, display: "block" }}>
                Suporta HTML. Ex: {"<h2>Coleta de Dados</h2><p>...</p>"}
              </Typography>
            </Grid>
          </Grid>
        </TabPanel>
