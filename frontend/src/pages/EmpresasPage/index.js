...
const EmpresasPage = () => {
  ...
  const handleImpersonate = async (company) => {
    try {
      // Salva token original ANTES da chamada
      const originalToken = localStorage.getItem("token");

      const { data } = await api.post("/auth/impersonate", { targetCompanyId: company.id });
      ...
      // Salva dados de impersonação para o banner e para sair depois
      ...
      // Atualiza token no localStorage E no header do axios
      localStorage.setItem("token", data.token);
      api.defaults.headers.Authorization = `Bearer ${data.token}`;

      toast.success(`Acessando empresa: ${company.name}`);
      // Redireciona sem recarregar a página para preservar o contexto do axios
      window.location.replace("/painel");
    } catch (err) {
      toast.error(err?.response?.data?.error || "Erro ao acessar empresa");
    }
  };
                  ...
                  <TableRow key={company.id} style={{ cursor: "pointer" }} onClick={() => handleEditCompany(company)}>
                    ...
                    <TableCell align="center">
                      <Box display="flex" justifyContent="center" style={{ gap: 4 }}>
                        ...
                        {user.super && company.id !== 1 && (
                          <IconButton
                            size="small"
                            onClick={(e) => { e.stopPropagation(); handleImpersonate(company); }}
                            style={{ color: "#c53030", backgroundColor: "#fff5f5", borderRadius: 6 }}
