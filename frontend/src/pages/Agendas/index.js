...
const UserSchedules = () => {
  ...
  const userTypeLower = (loggedUser?.userType || "").toLowerCase();
  const profileLower = (loggedUser?.profile || "").toLowerCase();
  const isAdminOrManager = ["administrador", "gerente", "manager", "administrator", "admin"].includes(userTypeLower)
    || profileLower === "admin";
  ...
  const canCreateSchedule = isAdminOrManager;
