...
import CalendarioAtendimento from "../../components/CalendarioAtendimento";
...
const Dashboard = () => {
  ...
  const loadCompanyPlan = async () => {
    if (!user?.companyId) return;
    try {
      const company = await findCompanies(user.companyId);
      if (company) {
        setCompanyPlan({
          planName: company.planName || company.plan?.name || "Plano Básico",
          startDate: company.createdAt ? moment(company.createdAt).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD'),
          dueDate: company.dueDate || moment().add(30, 'days').format('YYYY-MM-DD'),
  ...
  // Oculta seção admin somente para quem explicitamente é atendente/profissional.
  // userType null (usuários antigos sem o campo) recebem acesso total.
  const isAtendenteProfissional = ["atendente", "profissional"].includes((user?.userType || "").toLowerCase());
  const isAdministrador = !isAtendenteProfissional;
  ...
  return (
    <div className={classes.container}>
      ...
      {/* Banner Slider — apenas administrador */}
      {isAdministrador && hasSlides && (
      ...
      {/* Plan Info Cards — apenas administrador */}
      {isAdministrador && <Grid container spacing={2} style={{ marginBottom: 24 }}>
      ...
      {/* Dashboard Metrics — oculto para atendente/profissional */}
      {!isAtendenteProfissional && <Typography className={classes.sectionTitle}>Métricas Principais</Typography>}

      {!isAtendenteProfissional && <Grid container spacing={2} style={{ marginBottom: 24 }}>
        <Grid item xs={12} sm={6} md={3}>
          <Card className={classes.indicatorCard}>
            <CardContent>
      ...
      {/* Calendário de Compromissos e Lembretes */}
      <Typography className={classes.sectionTitle}>Calendário</Typography>
      <div style={{ marginBottom: 24 }}>
        <CalendarioAtendimento filterByUser={isAtendenteProfissional} />
      </div>
    </div>
  );
};
